public struct FocusCmdArgs: CmdArgs {
    ...
    public static let parser: CmdParser<Self> = .init(
        ...
        flags: [
            "--ignore-floating": falseBoolFlag(\Self.floatingAsTiling),
            "--window-id": windowIdSubArgParser(keyPath: \Self.windowId),
            "--dfs-index": singleValueSubArgParser(\Self.dfsIndex, "<dfs-index>", parseUInt32),
            ...
            "--wrap-around": trueBoolFlag(\Self.wrapAroundAlias),
        ],
        posArgs: [ArgParser(\Self.cardinalOrDfsDirection, upcastArgParserFun(parseCardinalOrDfsDirection))],
        conflictingOptions: [
